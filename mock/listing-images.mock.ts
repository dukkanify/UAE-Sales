...
/**
 * Extra catalog rows (hyundai-accent-2, kia-sportage-1, …) are generated by
 * template. Without a slug-specific gallery they fell through to the `default`
 * pool (Mercedes GT + Dubai skyline + interior). Keep product photos aligned
 * with the listing title.
 */
const extraListingPhotosByPrefix: Array<{
  prefix: string;
  ids: readonly string[];
}> = [
  {
    prefix: "kia-sportage-",
    ids: [
      "photo-1649921777129-a28a26031a03",
      "photo-1688893287874-ac7fbd686c24",
    ],
  },
  {
    prefix: "hyundai-accent-",
    ids: [
      "photo-1679943148492-4be81fd6ad0a",
      "photo-1550355291-bbee04a92027",
    ],
  },
  {
    prefix: "1br-apartment-offer-",
    ids: [
      "photo-1502672260266-1c1ef2d93688",
      "photo-1522708323590-d24dbb6b0267",
    ],
...
function extraPhotosForSlug(slug: string): string[] | undefined {
  const match = extraListingPhotosByPrefix.find((entry) =>
    slug.startsWith(entry.prefix),
  );
  return match ? photosFromIds(match.ids) : undefined;
}
...
export function imagesForSlug(slug: string, categoryId?: string): string[] {
  if (listingImages[slug]?.length) {
    return listingImages[slug];
  }

  const extra = extraPhotosForSlug(slug);
  if (extra?.length) {
    return extra;
  }

  const category =
    slugCategories[slug] ?? resolveFallbackCategory(categoryId);
  return galleryFromPool(category, slug, 3);
}
