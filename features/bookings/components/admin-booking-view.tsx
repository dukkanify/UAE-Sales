...
function AdminBookingView({ roleLabel }: AdminBookingViewProps) {
  ...
  const pendingPatchRef = React.useRef<Partial<BookingSettings>>({});
  ...
  async function flushSettings(patch: Partial<BookingSettings>) {
    setSaving(true);
  ...
  function queueSettings(patch: Partial<BookingSettings>) {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      void flushSettings(body);
    }, 320);
  }

  async function setStatus(id: string, status: string) {
    setBusyId(id);
    const res = await bookingJson<BookingListItem>("/api/bookings/" + id, "PATCH", { status });
    setBusyId(null);
    if (!res.success || !res.data) {
          ...
          <div className="space-y-5 rounded-2xl border border-border/60 bg-card p-5 sm:p-6">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              <ToggleRow
                ...
                onChange={(v) => queueSettings({ enabled: v })}
              />
              <ToggleRow
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <Field
                ...
                onCommit={(v) => queueSettings({ slotDurationMinutes: Number(v) })}
              />
              <Field
                label="Max advance days"
                type="number"
                value={settings.maxAdvanceDays}
                onCommit={(v) => queueSettings({ maxAdvanceDays: Number(v) })}
              />
              <Field
                label="Min notice (min)"
                type="number"
                value={settings.minNoticeMinutes}
                onCommit={(v) => queueSettings({ minNoticeMinutes: Number(v) })}
              />
            </div>
            ...
            {!settings.aroundTheClock ? (
              <div className="grid gap-4 sm:grid-cols-2">
                <Field
                  ...
                  onCommit={(v) => queueSettings({ dayStartHour: Number(v) })}
                />
                <Field
                  label="Day end hour (1–24)"
                  type="number"
                  value={settings.dayEndHour}
                  onCommit={(v) => queueSettings({ dayEndHour: Number(v) })}
                />
              </div>
            ) : null}
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        queueSettings({ instructorIds: next });
                      }}
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {b.status === "pending" ? (
                    ...
                    >
                      {busyId === b.id ? "Working…" : "Confirm + Zoom"}
                    </Button>
                  ) : null}
                  {b.status === "confirmed" ? (
                    <Button
